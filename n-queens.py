...
def startGen(gen, size, popSize, pop):
    ...
    for i in range(gen):  # numero de geracoes
        ...
        order = sorted(pop, reverse=True)
        bests.append(order[0])

        """ if(order[0][0] == size):    condição de parada nao necessaria para o tabuleiro valorado
            break """
        ...
        popOrdered = sorted(pop, reverse=True)
        temp = []
        for j in range(popSize):
            temp.append(popOrdered[j])

        # retira os n piores elementos da populacao
        pop = [x for x in temp if x in pop]
...
def pmxCross(pop, size, parent1, parent2):
    ...
    child1 = recursion1(temp_child1, point1, point2, parent1,
                        parent1Mid, parent2, parent2Mid, relations)
    child2 = recursion2(temp_child2, point1, point2, parent1,
                        parent1Mid, parent2, parent2Mid, relations)
...
def recursion1(temp_child, point1, point2, parent1, parent1Mid, parent2, parent2Mid, relations):
    ...
    if len(child) > len(child_unique):
        child = recursion1(child, point1, point2, parent1,
                           parent1Mid, parent2, parent2Mid, relations)
    return(child)
...
def recursion2(temp_child, point1, point2, parent1, parent1Mid, parent2, parent2Mid, relations):
    ...
    child_unique = np.unique(child)
    if len(child) > len(child_unique):
        child = recursion2(child, point1, point2, parent1,
                           parent1Mid, parent2, parent2Mid, relations)
    return(child)
...
matriz = matriz = np.zeros((size, size), dtype=float)
for i in range(size):
    for j in range(size):
        if (i % 2 == 0):
...
for i in range(1):
    ...
    for i in range(len(bests)):
        x.append(float(i))
        y.append(float(bests[i][0]))

    bestFit = [lastGen[0]]
    print(bestFit)
    for i in range(1, popSize):
        if(lastGen[i][0] == bestFit[0][0] and len(bestFit) < 10 and lastGen[i] not in bestFit):
            bestFit.append(lastGen[i])

    print("O melhor fitness encontrado nessa execução é: ", lastGen[0][0])
    print("Geração final: ", len(bests))
